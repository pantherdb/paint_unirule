...
public class Preferences {
	...
	private static final long serialVersionUID = -5472475387423113108L;
        public static final Color COLOR_WARNING_DEV = new Color(255, 204, 204); 
	...
	private String pantherURL = null;
        private String uriprotURLDev = null;
        private String uriprotURLProd = null;
        private String keyUniprotConnect = null;
        ...
        public static final String KEY_URL_SERVLET = "servlet_url";
        public static final String KEY_URL_UNIPROT_PROD = "uniprot_url_prod";
        public static final String KEY_URL_UNIPROT_DEV = "uniprot_url_dev";
        public static final String KEY_URL_UNIPROT_CONNECT = "uniprot_url_connect";        
        public static final String KEY_TREE_SCALE = "tree_scaling";
        public UniprotURLConnect uniprotUrlConnect = UniprotURLConnect.PRODUCTION;
        public static final String UNIPROT_URL_START_UNIRULE = "/unirule/";
        ...
        private  ReadResources initResources() {
            try {
                ReadResources rr = new ReadResources(PROPERTY_USER);
                pantherURL = rr.getKey(KEY_URL_SERVLET);
                uriprotURLProd = rr.getKey(KEY_URL_UNIPROT_PROD);
                uriprotURLDev =  rr.getKey(KEY_URL_UNIPROT_DEV);
                keyUniprotConnect = rr.getKey(KEY_URL_UNIPROT_CONNECT);
                if (null != keyUniprotConnect && keyUniprotConnect.equals(KEY_URL_UNIPROT_DEV)) {
                    keyUniprotConnect = KEY_URL_UNIPROT_DEV;
                    uniprotURL = uriprotURLDev; 
                    uniprotUrlConnect = UniprotURLConnect.DEVELOPMENT;
                }
                else {
                    keyUniprotConnect = KEY_URL_UNIPROT_PROD;
                    uniprotURL = uriprotURLProd; 
                    uniprotUrlConnect = UniprotURLConnect.PRODUCTION;
                }
                tree_distance_scaling = Double.parseDouble(rr.getKey(KEY_TREE_SCALE));
                return rr;
            }
            catch(Exception e) {
                e.printStackTrace();
    ...
    public boolean save() {
        try {
            ...
            props.setProperty(KEY_URL_SERVLET, pantherURL);
            props.setProperty(KEY_URL_UNIPROT_PROD, uriprotURLProd);
            props.setProperty(KEY_URL_UNIPROT_DEV, uriprotURLDev);
            props.setProperty(KEY_URL_UNIPROT_CONNECT, keyUniprotConnect);            
            props.setProperty(KEY_TREE_SCALE, Double.toString(tree_distance_scaling));
...
//	private String uploadVersion = "dev_3_panther_upl|UPL 10.0";
	...
	private Color bpPaintColor = new Color(255, 248, 220);
        private Color urPaintColor = new Color(243, 199, 208);
...
//	}
    ...
    public String getUriprotURLPrefix() {
        if (null != uniprotURL) {
            int index = uniprotURL.indexOf(UNIPROT_URL_START_UNIRULE);
            if (index >=0) {
                return uniprotURL.substring(0, index);
            }
        }
        return null;
    }
...
//	}
    ...
    public Color getUrPaintColor() {
        return urPaintColor;
    }

    public void setUrPaintColor(Color urPaintColor) {
        this.urPaintColor = urPaintColor;
    }
    ...
    public Color getNonExpnonPaintColor() {
        return nonExpnonPaintColor;
    }

    public void setNonExpnonPaintColor(Color nonExpnonPaintColor) {
        this.nonExpnonPaintColor = nonExpnonPaintColor;
    }
...
//	}
    ...
    public UniprotURLConnect getUniprotUrlConnect() {
        return uniprotUrlConnect;
    }

    public void setUniprotUrlConnect(UniprotURLConnect uniprotUrlConnect) {
        this.uniprotUrlConnect = uniprotUrlConnect;
        if (uniprotUrlConnect.PRODUCTION == this.uniprotUrlConnect) {
            uniprotURL = uriprotURLProd;
            keyUniprotConnect = KEY_URL_UNIPROT_PROD;
