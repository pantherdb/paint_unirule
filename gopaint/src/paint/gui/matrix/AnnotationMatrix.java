...
//		manager.registerTermListener(this);
		...
		setDefaultRenderer(NodeInfoForMatrix.class, matrix_renderer);
                setDefaultRenderer(NodeInfoForUniRuleMatrix.class, matrix_renderer);
	...
	public void setModels(List<GeneNode> orderedNodes, MatrixInfo mi) {
		...
		models.clear();
                if (null == mi) {
                    ...
                    AnnotationMatrixModel amm = (AnnotationMatrixModel)tm;
                    amm.clear();
                    return;
                    
                }
		AnnotationMatrixModel annot_model;
		annot_model = new AnnotationMatrixModel(orderedNodes, AnnotationTypeSelector.AnnotationType.BIOLOGICAL_PROCESS.toString(), mi.getGroups(AnnotationTypeSelector.aspects.get(AnnotationTypeSelector.AnnotationType.BIOLOGICAL_PROCESS.toString())));
		models.put(AnnotationTypeSelector.AnnotationType.BIOLOGICAL_PROCESS.toString(), annot_model);
		annot_model = new AnnotationMatrixModel(orderedNodes, AnnotationTypeSelector.AnnotationType.CELLULAR_COMPONENT.toString(), mi.getGroups(AnnotationTypeSelector.aspects.get(AnnotationTypeSelector.AnnotationType.CELLULAR_COMPONENT.toString())));
		models.put(AnnotationTypeSelector.AnnotationType.CELLULAR_COMPONENT.toString(), annot_model);
		annot_model = new AnnotationMatrixModel(orderedNodes, AnnotationTypeSelector.AnnotationType.MOLECULAR_FUNCTION.toString(), mi.getGroups(AnnotationTypeSelector.aspects.get(AnnotationTypeSelector.AnnotationType.MOLECULAR_FUNCTION.toString())));
		models.put(AnnotationTypeSelector.AnnotationType.MOLECULAR_FUNCTION.toString(), annot_model);
                
                UniRuleMatrix urm = new UniRuleMatrix();
                annot_model = new AnnotationMatrixModel(urm);
                models.put(AnnotationTypeSelector.AnnotationType.UNIRULE.toString(), annot_model);
                
		String annotationType = AnnotationTypeSelector.inst().getAnnotationType().toString();
		AnnotationMatrixModel matrix = models.get(annotationType);
		setModel(matrix);
    ...
    public void mousePressed(MouseEvent event) {
        ...
        AnnotationTypeSelector.AnnotationType at = AnnotationTypeSelector.inst().getAnnotationType();
...
//				}
            ...
            if (event.isMetaDown() && !event.isShiftDown() && !event.isAltDown() && !event.isControlDown()) {
                ...
                setSelectedColumn(column);
                this.clickedRow = row;
                System.out.println("Right click column selected");
                // GO TERM selection
                if (AnnotationTypeSelector.AnnotationType.MOLECULAR_FUNCTION.equals(at)
                        || AnnotationTypeSelector.AnnotationType.BIOLOGICAL_PROCESS.equals(at)
                        || AnnotationTypeSelector.AnnotationType.CELLULAR_COMPONENT.equals(at)) {
                    TermAncestor termAncestor = ((AnnotationMatrixModel) getModel()).getTermAncestorAtColumn(column);
                    if (termAncestor != null) {
                ...
                else if (AnnotationTypeSelector.AnnotationType.UNIRULE.equals(at)) {
                    ...
                    TreePanel tree_pane = pm.getTree();
                    List<GeneNode> geneNodeList = new ArrayList<GeneNode>();
                    ...
                    for (GeneNode gNode : allNodes) {
                        ...
                        for (UAnnotation a : annotList) {
                            if (id.equals(a.getRule().getId()) && Evidence.isExperimental(a.getEvCode())) {
                                geneNodeList.add(gNode);
                                break;
                            }
                        }
                    }
                    selected_genes = geneNodeList;
                    selectedNode = getMRCA(selected_genes);
                }
            }
...
//                                    EventManager.inst().fireGeneEvent(ge);
        ...
        // Drag handling only for UniRule           
//        if (AnnotationTypeSelector.AnnotationType.MOLECULAR_FUNCTION.equals(at)
//                || AnnotationTypeSelector.AnnotationType.BIOLOGICAL_PROCESS.equals(at)
//                || AnnotationTypeSelector.AnnotationType.CELLULAR_COMPONENT.equals(at)) {
//            return;
//        }
        // Only permit annotation of new Unirules and allowed types
        int column = columnAtPoint(point);       
        if (false == model.canCurate(column)) {
            return;
        }
...
//    public void mousePressedOrig(MouseEvent event) {
//        Point point = event.getPoint();
//        int row = rowAtPoint(point);
//        int column = columnAtPoint(point);
//        AnnotationMatrixModel model = (AnnotationMatrixModel) this.getModel();
...
//
//            }
//            // Annotation to GO terms not permitted
////                System.out.println("Going to export as drag");
////		annot_handler.exportAsDrag(this, event, TransferHandler.COPY);
//            return;
//        }
...
//                    } else if (previousCopy.size() != selection.size()) {
//                        updateRows(previousCopy);
//                        updateRows(selection);
//                    } else {
//                        boolean need_update = false;
//                        for (GeneNode gene : previousCopy) {
//                            need_update |= !selection.contains(gene);
//                        }
//                        if (need_update) {
//                            updateRows(previousCopy);
//                            updateRows(selection);
...
//            }
//
//            System.out.println("Unirule drag");
//            annot_handler.exportAsDrag(this, event, TransferHandler.COPY);
//
//        }
//
//    }
	...
	public void handleAspectChangeEvent(AspectChangeEvent event) {
		...
		if (models != null && 0 != models.size()) {
			String go_aspect = AnnotationTypeSelector.inst().getAnnotationType().toString();
    ...
    public void handleGeneSelectEvent(GeneSelectEvent event) {
        ...
        this.clearSelection();
        if (false == this.getModel() instanceof AnnotationMatrixModel) {
            return;
        }
        AnnotationMatrixModel model = (AnnotationMatrixModel) this.getModel();
...
//            }
    ...
    public AnnotationMatrixModel getAnnotationMatrixModel(String type) {
        if (null == models) {
            return null;
        }
    ...
    public void close() {
        setModels(new ArrayList<GeneNode>(), null);
