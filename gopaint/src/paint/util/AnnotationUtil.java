...
//    }
    ...
    public static boolean hasDirectNot(GeneNode gNode, GOTermHelper gth) {
        ...
        String aspect = AnnotationTypeSelector.aspects.get(AnnotationTypeSelector.inst().getAnnotationType().toString());

        if (true == aspect.equals(AnnotationTypeSelector.aspects.get(AnnotationTypeSelector.AnnotationType.BIOLOGICAL_PROCESS.toString()))
                || true == aspect.equals(AnnotationTypeSelector.aspects.get(AnnotationTypeSelector.AnnotationType.MOLECULAR_FUNCTION.toString()))
                || true == aspect.equals(AnnotationTypeSelector.aspects.get(AnnotationTypeSelector.AnnotationType.CELLULAR_COMPONENT.toString()))) {
            ...
            for (Annotation annot : annotList) {
                if (false == aspect.equals(gth.getTerm(annot.getGoTerm()).getAspect())) {
                    continue;
                }
        ...
        for (UAnnotation uAnnot: uAnnotList) {
            if (true == QualifierDif.containsNegative(uAnnot.getQualifierSet())) {
                return true;
            }
        }
        return false;
    ...
    public static void graftBranch(GeneNode gNode) {
        ...
        UAnnotationHelper.fixAnnotationsForGraftPruneOperation(pm.getTree().getRoot().getNode(), gNode.getNode());        
        branchNotify(gNode);
    }
    ...
    public static void pruneBranch(GeneNode gNode) {
        ...
        for (GeneNode desc : descendents) {
            ...
            // Unprune any descendants 
            if (nvi.isPruned() && gNode!= desc) {
                nvi.setPruned(false);
            }
            ArrayList<Annotation> annotList = nvi.getGoAnnotationList();
            if (null != annotList) {
            ...
            // repeat for uAnnotation
            ArrayList<UAnnotation> uAnnotList = nvi.getuAnnotationList();
            if (null == uAnnotList) {
                continue;
            }
            for (Iterator<UAnnotation> annotIter = uAnnotList.iterator(); annotIter.hasNext();) {
                UAnnotation a = annotIter.next();
                if (true == UAnnotationHelper.isPAINTAnnotation(a)) {
                    annotIter.remove();
                }
            }
            if (true == uAnnotList.isEmpty()) {
