...
//        }
        ...
        if (isTerminus()) {
            ...
            g.setFont(f);
            String s = getNodeLabelWithAccPrefix();
            if (isPruned()) {
                s = "XXX-" + s;
...
//        this.blocked = prune;
    ...
    public String getNodeLabelWithAccPrefix() {
        String sfInfo = Constant.STR_EMPTY;
        NodeVariableInfo nvi = node.getVariableInfo();
        if (null != nvi) {
            SFAnnotation sfAnnot = nvi.getSfAnnotation();
            if (null != sfAnnot) {
                String acc = nvi.getSfAccWithoutBookPart();
                String label = sfAnnot.getSfLabel();
                if (null != acc && null != label) {
                    sfInfo = acc + Constant.STR_COLON + label;
                }
            }
        }
        String label = getNodeLabelProt();
        if (null != label && false == Constant.STR_EMPTY.equals(sfInfo)) {
            return label + Constant.STR_UNDERSCORE + sfInfo;
        }
        else if (null == label && Constant.STR_EMPTY.equals(sfInfo)) {
    ...
    //  For Uniprot, use protein id and species.
    public String getNodeLabelProt() {
        String s = null;
        if (isLeaf()) {
            s = Node.getProteinIdFromLongGeneName(node);
            if (null != s) {
                s = s + STR_UNDERSCORE + node.getStaticInfo().getSpecies();
            }
            return s;
        }
        return node.getStaticInfo().getSpecies();
    }
...
//            }
    ...
    public String getNodeLabelWithPTNAndAcc() {
        String s = null;
        NodeStaticInfo nsi = node.getStaticInfo();
        if (isLeaf()) {
            s = Node.getProteinIdFromLongGeneName(node);
        ...
        return s + STR_UNDERSCORE + nsi.getAccWithoutBookPart() + STR_UNDERSCORE + nsi.getPublicId();
    }    
...
//    }
    ...
    public boolean hasUniRuleEvidence() {
        Node n = this.getNode();
