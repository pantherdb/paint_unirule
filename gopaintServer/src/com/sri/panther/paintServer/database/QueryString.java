...
//    public static final String PREPARED_SEARCH_BOOKS_BY_DEFINITION = "select distinct c.ACCESSION, c.NAME, u.NAME, u.EMAIL, cst.STATUS, cst.STATUS_TYPE_SID, u.login_name  from view_protein_classification vpc, classification_relationship cr, classification c, curation_status cs, curation_status_type cst, users u where (vpc.PROTEIN_ID) in (select protein_id from identifier i where i.identifier_type_sid = 3 and i.name like ?) and vpc.CLASSIFICATION_ID = cr.CHILD_CLASSIFICATION_ID  and cr.PARENT_CLASSIFICATION_ID = c.CLASSIFICATION_ID and c.DEPTH = ? and c.CLASSIFICATION_VERSION_SID = ? and c.CLASSIFICATION_ID = cs.CLASSIFICATION_ID (+) and cs.CURATION_STATUS_ID = cst.STATUS_TYPE_SID (+) and cs.USER_ID = u.USER_ID (+) order by c.ACCESSION";
    ...
    public static final String PANTHER_TREE_STRUCTURE = "select n1.accession as child_accession, n1.node_id as child_id, n1.public_id as child_public_id, n2.accession as parent_accession, n2.node_id as parent_id, n2.public_id as parent_public_id from node n1, node n2, node_relationship nr where n1.CLASSIFICATION_VERSION_SID = %1 and n1.CLASSIFICATION_VERSION_SID = n2.CLASSIFICATION_VERSION_SID and n1.accession like '%2' and n1.node_id = nr.CHILD_NODE_ID and nr.parent_node_id = n2.NODE_ID ";

    public static final String NODE_INFO =  "select n.accession, np.accession as parent_accession, n.PUBLIC_ID, np.public_id as parent_public_id, n.branch_length, np.branch_length as parent_branch_length, nt.node_type, et.event_type, g.PRIMARY_EXT_ACC\n" +
                                                "from node n\n" +
                                                "join node_type nt on n.node_type_id = nt.NODE_TYPE_ID\n" +
                                                "left join event_type et on n.EVENT_TYPE_ID = et.EVENT_TYPE_ID \n" +
                                                "join node_relationship nr on n.node_id = nr.child_node_id\n" +
                                                "join node np on nr.parent_node_id = np.node_id \n" +
                                                "left join gene_node gn on n.node_id = gn.node_id \n" +
                                                "left join gene g on  gn.gene_id = g.gene_id\n" +
                                                "where n.accession like '%1' and n.CLASSIFICATION_VERSION_SID = %2\n" +
                                                "and n.OBSOLESCENCE_DATE is null and np.OBSOLESCENCE_DATE is null and nr.OBSOLESCENCE_DATE is null and g.OBSOLESCENCE_DATE is null and gn.OBSOLESCENCE_DATE is null ";
